...
def route(user_query: str, intent: str, current_date: str | None = None, release_track: str | None = None) -> RetrievalRoute:
    ...
    # Mechanics rules: prefer tierA research sources first.
    if intent in {"magic_find_rule", "treasure_class_rule", "affix_level_rule", "charm_rule", "crafting_rule"}:
        entities = _extract_entities(user_query)
        keywords = entities if entities else [user_query]
        sites = [
            "theamazonbasin.com",
        ...
        return RetrievalRoute(
            need_retrieval=True,
            reason="机制规则属于强事实：优先 Tier A（Basin/DiabloWiki），必要时 Tier B（PureDiablo/工具）补证。",
            query_plan=QueryPlan(keywords=keywords, sites=sites, as_of_date=current_date),
            expected_entities=entities,
        )

    # Mechanics claims: simple class/weapon interaction questions should answer directly first.
    if intent == "mechanics_claim":
        entities = _extract_entities(user_query)

        simple_direct_claim = any([
            ("双手" in user_query and "单手" in user_query),
            ("two-handed" in q and "one" in q and "hand" in q),
            ("levitate" in q),
            ("levitation" in q),
        ])
        ...
        if simple_direct_claim:
            return RetrievalRoute(
                ...
                reason="简单机制判断题：先直接回答已知职业/武器交互规则；只有需要补精确例外/版本证据时再检索。",
                query_plan=None,
                expected_entities=entities,
            )

        # For more ambiguous mechanics claims, keep a retrieval path available.
        keywords: list[str] = []
        if "levitate" in q:
        ...
        return RetrievalRoute(
            need_retrieval=True,
            reason="复杂机制/硬规则题：需要官方或资料站证据交叉验证。",
            query_plan=QueryPlan(keywords=keywords, sites=sites, as_of_date=current_date),
            expected_entities=entities,
        )
