...
class Trace(BaseModel):
    ...
    # Intent classification traceability (v2)
    rule_intent: str = ""
    final_intent: str = ""
    intent_source: str = "rule"  # "rule" | "llm_fallback"
    fallback_confidence: Optional[str] = None
    fallback_reason_code: Optional[str] = None

    # Mechanics upgrade fields (Phase 0+)
    mechanics_fact_hits: int = 0
    mechanics_topics_used: list[str] = Field(default_factory=list)
    rules_applied: list[str] = Field(default_factory=list)
    formulas_used: list[str] = Field(default_factory=list)
    source_tiers_used: list[str] = Field(default_factory=list)
    conflict_detected: bool = False
    followup_fields_requested: list[str] = Field(default_factory=list)
